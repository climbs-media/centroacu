...
@Component({
  selector: 'app-detalles-ejercicios-pacientes-user',
  templateUrl: './detalles-ejercicios-pacientes-user.page.html',
  styleUrls: ['./detalles-ejercicios-pacientes-user.page.scss'],
})
export class DetallesEjerciciosPacientesUserPage implements OnInit {
        ...
        },
        {
          ...
          handler: () => {
            this.diarioService.borrarAnadirEjercicio(this.item.id)
              .then(
                res => {
                  this.router.navigate(['/diario-ejercicios-user']);
                },
                err => console.log(err)
              );
          }
